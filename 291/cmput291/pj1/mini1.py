...
conn = sqlite3.connect("mini1.db")
c = conn.cursor()

def create_table():
    global conn, c
    createtable='''drop table if exists deliveries;
...
def main():
    create_table()
    insert_data()
    print("type LOGOUT to log out at any time. ")
    print("type QUIT to quit program")
    
    a_s = input("Are you an agent?(Y/N)")
    leave(a_s)
    if a_s == 'Y' or a_s =='y':
        #get ID and password fromm agents
        entID = input("Please enter your identification. \n")
        leave(a_s)
        c.execute ("SELECT pwd FROM agents WHERE cid = ?", (entID,))
        tule = c.fetchone()
        getPassword = tule[0]
        ...
        #getch id and password from customers
        entID = input("Please enter your identification. ")
        leave(entID)
        c.execute ("SELECT pwd FROM customers WHERE cid = ?", (entID,))
        tule = c.fetchone()
        #if id and password doesnt exist
        if tule == None:
            newcusBool = input("You are not registered, do you want to sign up right now?(Y/N)")
            leave(newcusBool)
            if newcusBool == 'Y' or newcusBool == 'y':
                newID = input("Please enter your unique ID.")
                leave(newID)
                #ask to enter again if not unique
                c.execute('SELECT name FROM customers WHERE cid = ?',(newID,))
                data = c.fetchone()
                while data != None:
                    newID = input("Please enter a unique ID again.")
                    leave(newID)
                    #ask to enter again if not unique
                    c.execute('SELECT name FROM customers WHERE cid =? ', (newID,))
                    data = c.fetchone()
                ...
                leave(newName)
                newAddr = input("Please enter your address.")
                leave(newAddr)
                newPwd = input("Please enter your password.")
                leave(newPwd)
                c.execute ('INSERT INTO customers (cid, name, address, pwd) VALUES (?,?,?,?);',(newID,newName,newAddr,newPwd))
            elif newcusBool == 'n' or newcusBool == 'N':
                print("fuck off")
                sys.exit(0)
        elif tule != None:
            getPassword=tule[0]
